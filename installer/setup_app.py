...
def gui_wizard() -> int:
    ...
    tk.Label(f0, text=f"{APP_NAME} {APP_VERSION}\nنسخه نهایی v4 — تیرا ایجنت تمام‌عیار + محصولات متنوع\nجاروبرقی، یخچال، لباسشویی، پراید، موبایل، سری 13 14 15 + دستورات اجرایی\nدیوار + شیپور + شکارچی هوشمند + IP ریست + ملی‌پیامک کامل + روبیکا اطلاع‌رسانی", font=("Segoe UI", 11), bg="white", fg="#334", justify="center").pack(pady=10)
    # ویژگی‌ها با آیکون
    feat_frame = tk.Frame(f0, bg="white")
    feat_frame.pack(pady=15)
    features = [
        "📦 یک فایل تکی رمزنگاری شده — بدون نیاز به اینترنت (1-2GB)",
        "🌐 کرومیوم اختصاصی + مدل تیرا داخل فایل نصب",
        "🧠 تیرا v4: جاروبرقی بوش، یخچال، پراید، سری 13 14 15، هرچی موبایل",
        "💬 متن پیامک رو بذار، متن چت، راهنمای ملی‌پیامک کامل",
        "🤖 دستورات از طریق ربات‌های بله/روبیکا/تلگرام → تیرا",
        "📱 هرچی موبایل: پیام خودکار + استخراج شماره‌ها + اطلاع روبیکا",
        "📨 ملی‌پیامک کامل: SendSMS + Pattern + Delivery + Inbox + Credit",
        "🛡️ IP ریست خودکار + سودآوری هزار پارامتری + مذاکره هوشمند",
    ]
