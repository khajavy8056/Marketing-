...
APP_NAME_FA = "مارکتینگ دیوار — تیرا نهایی"
VERSION = "3.5.0-tira-final"
    ...
    def work():
        ...
        try:
            ...
            try:
                ...
                pack(offline=True, encrypt=True)  # رمزنگاری شده برای Setup تکی
                ...
                log(f"Pack error: {e}")
                _run_with_log(python_cmd + ["installer/pack_payload.py", "--all"], log, cwd=ROOT)
                set_progress(bar_pack, lbl_pack, 100, "4️⃣ بسته‌بندی کامل ✅")
            ...
            # اول payload رمز شده را چک کن، بعد ساده
            payload_path = ROOT / "installer" / "payload.zip.enc"
...
def cli():
    ...
    subprocess.run(python_cmd + ["installer/pack_payload.py", "--all"], cwd=str(ROOT))
    return 0
