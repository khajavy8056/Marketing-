...
try:
    ...
    def fetch_market_price_from_web(product, timeout=8, use_cache=True):
        return None
...
def get_market_price_for_model(model_name: str) -> Optional[int]:
    """قیمت بازار از اینترنت برای یک مدل — ترب + کش."""
    try:
        # تمیز کردن نام برای جستجو
        q = model_name.replace("آیفون", "iphone").strip()
        # برای آیفون، نات‌اکتیو / کارکرده را جدا تشخیص بده
        is_not_active = any(w in model_name.lower() for w in ["نات اکتیو", "not active", "پلمپ"])
        prod = {"keyword": q, "model": model_name, "brand": "apple", "category": "mobile-phones"}
        price = fetch_market_price_from_web(prod, timeout=6, use_cache=True)
        if price:
            # اگر نات‌اکتیو است، قیمت معمولاً بالاتر — ولی ترب قیمت نو می‌دهد، ما همون را برمی‌گردانیم
            return int(price)
    except Exception:
        pass
    return None

def fetch_all_market_prices(models: List[str]) -> Dict[str, int]:
    out = {}
    for m in models:
        try:
            p = get_market_price_for_model(m)
            if p:
                out[m] = p
        except Exception:
            pass
...
def generate_ai_message(step: str, context: Dict[str, Any]) -> str:
    ...
    if step == "ask_sell_price":
        ...
        # اگر قیمت بازار از اینترنت داریم، پیشنهاد بده
        try:
            mp = get_market_price_for_model(model)
            if mp:
                # قیمت را به میلیون نمایش بده
                mp_m = mp // 1_000_000
                base = _pick(ASK_SELL_PRICE).format(model=model)
                return base + f" (قیمت بازار ترب ~{mp_m} میلیون — اگر تمیز باشه، تو چقدر می‌فروشی؟)"
        except Exception:
            pass
