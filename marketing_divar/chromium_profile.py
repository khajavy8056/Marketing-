...
def cookies_per_platform(cookies: List[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    if sheypoor_cookies:
        ...
        # حداقل 2 کوکی یا کوکی auth-like با مقدار طولانی
        if len(sheypoor_cookies) >= 2:
            # اگر یکی مقدار طولانی دارد
            for c in sheypoor_cookies:
                v = str(c.get("value") or "")
                if len(v) > 20:
                    sheypoor_ok = True
                    break
            # حتی بدون مقدار طولانی، 3+ کوکی یعنی لاگین احتمالا
            if not sheypoor_ok and len(sheypoor_cookies) >= 3:
                sheypoor_ok = True
        if not sheypoor_ok:
            for hint in _SHEYPOOR_HINTS:
                if any(hint in n for n in names):
                    # چک مقدار
                    for c in sheypoor_cookies:
                        if hint in str(c.get("name") or "").lower() and len(str(c.get("value") or "")) > 10:
                            sheypoor_ok = True
                            break
                    if sheypoor_ok:
                        break
        ...
        if not sheypoor_ok:
            for c in sheypoor_cookies:
                v = str(c.get("value") or "")
                if len(v) > 30 and "session" in str(c.get("name") or "").lower():
                    sheypoor_ok = True
                    break
